                ...
                {txResult && (
                    <div className="mb-8">
                        ...
                        {txResult.decision === 'BLOCKED' && (
                            <div className="bg-orange-50 border-l-4 border-orange-400 p-4 mb-6 rounded-r shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
                                <div className="flex items-start gap-4">
                                    <div className="text-2xl">🚨</div>
                                    <div>
                                        <h3 className="text-orange-800 font-bold uppercase text-sm tracking-wider">Security Vulnerability Detected</h3>
                                        <p className="text-orange-700 text-xs mt-1">
                                            High-value transactions or unusual patterns are manually or automatically blocked for your protection.
                        ...
                        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-8">
                            <div className="flex justify-between items-start mb-8">
                                <div className="flex items-center gap-4">
                                    <div className={`w-12 h-12 rounded-full flex items-center justify-center text-2xl ${txResult.decision === 'APPROVED' ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600'}`}>
                                        {txResult.decision === 'APPROVED' ? '✓' : '✕'}
                                    </div>
                                    <div>
                                        <h2 className={`text-xl font-bold ${txResult.decision === 'APPROVED' ? 'text-green-700' : 'text-red-700'}`}>
                                            Transaction {txResult.decision === 'APPROVED' ? 'Authorized' : 'Declined'}
                                        </h2>
                                ...
                                <span className={`px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-widest border ${txResult.decision === 'APPROVED' ? 'bg-green-50 text-green-700 border-green-200' : 'bg-red-50 text-red-700 border-red-200'}`}>
                                    {txResult.decision}
                                </span>
                            </div>
                                    ...
                                    history.map((tx) => (
                                        <div key={tx.id} className="group border border-transparent hover:border-gray-100 rounded-xl transition-all">
                                            <div className="flex items-center justify-between p-4 hover:bg-gray-50 rounded-xl transition-colors">
                                                <div className="flex items-center gap-4">
                                                    <div className={`w-10 h-10 rounded-full flex items-center justify-center text-lg font-bold ${tx.decision === 'APPROVED' ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600'}`}>
                                                ...
                                                <div className="flex items-center gap-4">
                                                    <div className="text-right">
                                                        ...
                                                        <div className="flex items-center justify-end gap-1 mt-1">
                                                            <span className={`text-[10px] font-bold uppercase ${tx.status === 'UNDER_REVIEW' ? 'text-orange-500' : tx.decision === 'APPROVED' ? 'text-green-500' : 'text-red-500'}`}>
                                                                {tx.status === 'UNDER_REVIEW' ? 'UNDER REVIEW' : tx.decision}
                                                            </span>
                                                        </div>
                                                    </div>
                                                    {tx.decision === 'BLOCKED' && tx.status !== 'UNDER_REVIEW' && !tx.is_appealed && (
                                                        <button
                                                            onClick={() => {
                                                                setTxResult({ id: tx.id, decision: 'BLOCKED', explanation: 'Historical Transaction', merchant: tx.merchant, amount: tx.amount });
